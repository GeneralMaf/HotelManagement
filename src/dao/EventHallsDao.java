...
public class EventHallsDao {
    ...
    static final String sql_update="UPDATE `event_halls` SET `hall_name` = ?, `equipment` = ?, `status` = ? WHERE `event_halls`.`hall_id` = ?;";
...
//    Cette methode permet de creer une salle pour les evenements
    ...
    return DatabaseService.executeUpdate(sql_update,e.getHall_name(),e.getCapacity(),e.getEquipment(),e.getStatus());
    }
